...
function normalizeAbsoluteUrl(url?: string): string | undefined {
  ...
  try {
    ...
    return u.origin; // ensure only origin is used
  } catch {
...
function getBaseUrl(req: NextRequest): string {
  ...
  // Next.js provides a reliable origin via nextUrl
  try {
...
function buildRedirect(baseUrl: string, status: 'failed' | 'error', orderId?: string, details?: Record<string, any>): URL {
...
export async function POST(req: NextRequest) {
  ...
  const redirectUrl = buildRedirect(baseUrl, 'failed', orderId, responseData as any);
  console.debug('[callback/fail] Redirecting (POST) to:', redirectUrl.toString());
  return NextResponse.redirect(redirectUrl.toString(), { status: 303 });
  } catch (error) {
  console.error('Error in failure callback (POST):', error);
    const baseUrl = getBaseUrl(req);
    const errorUrl = buildRedirect(baseUrl, 'error');
  console.debug('[callback/fail] Redirecting (POST error) to:', errorUrl.toString());
...
export async function GET(req: NextRequest) {
  ...
  const redirectUrl = buildRedirect(baseUrl, 'failed', orderId || undefined, raw);
  console.debug('[callback/fail] Redirecting (GET) to:', redirectUrl.toString());
  return NextResponse.redirect(redirectUrl.toString(), { status: 303 });
  } catch (error) {
    console.error('Error in failure callback (GET):', error);
    const baseUrl = getBaseUrl(req);
    const errorUrl = buildRedirect(baseUrl, 'error');
  console.debug('[callback/fail] Redirecting (GET error) to:', errorUrl.toString());
  return NextResponse.redirect(errorUrl.toString(), { status: 303 });
  }
}